...
function getContent(fileSource, place) {
  fetch(fileSource)
    .then((resolvedVal) => {
    ...
    .then((content) => {
      for (let i = 0; i < content.length; i++) {
        ...
        let backTxt = document.createElement("p");
        ...
        //---------------------------//
        let frontTxt = document.createElement("span");
        frontTxt.textContent = content[i]["start"];
        //---------------------------//
        let mainContent = document.createElement("p");
        mainContent.textContent = content[i]["content"];
        //---------------------------//
        ...
        //---------------------------//
        let innerCounter = document.createElement("span");
        innerCounter.classList = "counter";
        innerCounter.setAttribute(
          "data-value",
          `${content[i]["iteration_number"]}`
        );
        innerCounter.textContent = `${content[i]["iteration_number"]}`;
...
getContent("azkar.json", morningContent);
// getContent("azkar.json", eveningContent);
// getContent("azkar.json", salahContent);
